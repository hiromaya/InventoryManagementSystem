...
try
{
    ...
    {
        Console.WriteLine($"PDFを開く際のエラー: {ex.Message}");
    }
}
catch (Exception ex)
{
    ...
    Console.WriteLine("  4. .NET 8.0 Windows Runtime がインストールされているか確認");
}
...
static void TestBasicReport()
{
    ...
    {
        Console.WriteLine($"❌ 基本レポートテストエラー: {ex.Message}");
        throw;
    }
}
...
static void TestMinimalReport()
{
    ...
    {
        Console.WriteLine($"❌ 最小レポートテストエラー: {ex.Message}");
        throw;
    }
}
...
static void TestJapaneseFontReport()
{
    try
    {
        ...
        // データバンド
        var dataBand = new DataBand { Height = 200 };
        page.Bands.Add(dataBand);
        
        var japaneseContent = new TextObject
        {
            Bounds = new System.Drawing.RectangleF(10, 10, 150, 180),
            Text = "📊 商品日報サンプル\n\n" +
                   "商品名：りんご（青森県産）\n" +
                   "商品コード：APPLE-001-AOM\n" +
                   "等級：特級\n" +
    ...
    {
        Console.WriteLine($"❌ 日本語フォントテストエラー: {ex.Message}");
        throw;
    }
}