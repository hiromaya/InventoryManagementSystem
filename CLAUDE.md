...
2. **Linux環境でのビルドエラー対応**
   - Linux環境でのビルドエラーは最小限の対応に留める
   - Windows環境でビルドエラーが出なければ、Linux環境のエラーは許容する
   - Windows環境でビルドエラーが発生した場合のみ、ユーザーからの修正依頼に基づいて対応する
...
**重要**: このプロジェクトは主にWindows環境で実行されることを前提としており、Linux環境（Claude Code）はあくまで開発補助ツールとしての位置づけです。
