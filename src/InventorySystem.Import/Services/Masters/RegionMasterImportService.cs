...
public class RegionMasterImportService : IRegionMasterImportService
{
    ...
    private readonly IRegionMasterRepository _repository;

    public RegionMasterImportService(
        ILogger<RegionMasterImportService> logger,
        IRegionMasterRepository repository)
    {
        _logger = logger;
    ...
    public async Task<InventorySystem.Core.Interfaces.ImportResult> ImportAsync(string filePath)
    {
        ...
        _logger.LogInformation("産地マスタCSV取込開始: {FilePath}", filePath);
        ...
        try
        {
            ...
            foreach (var (record, index) in records.Select((r, i) => (r, i + 1)))
            {
                try
                {
                    if (!record.IsValid())
                    {
                        _logger.LogWarning("行 {Index}: 無効なレコード - 産地コード: {Code}, 産地名: {Name}", 
                            index, record.RegionCode, record.RegionName);
                        errorCount++;
                        continue;
                    }
                    ...
                    // データベースに保存
                    await _repository.UpsertAsync(region);
                    
                    importedCount++;
                    _logger.LogDebug("行 {Index}: 産地マスタ登録完了 - {Code}: {Name}", 
                        index, record.RegionCode, record.RegionName);
                }
                catch (Exception ex)
                {
            ...
            var result = new InventorySystem.Core.Interfaces.ImportResult
            {
                ...
                Message = $"産地マスタ取込完了: 成功 {importedCount}件, エラー {errorCount}件",
        ...
        {
            var errorMessage = $"産地マスタCSV取込エラー: {ex.Message}";
            _logger.LogError(ex, errorMessage);
    ...
    /// CSVファイルを読み込む
    /// </summary>
    private async Task<List<RegionMasterCsv>> ReadCsvFileAsync(string filePath)
    {
        var records = new List<RegionMasterCsv>();
