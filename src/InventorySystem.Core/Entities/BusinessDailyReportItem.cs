namespace InventorySystem.Core.Entities
{
    public class BusinessDailyReportItem
    {
        ...
        public decimal DailySalesTotal => 
            (DailyCashSales ?? 0) + (DailyCashSalesTax ?? 0) + (DailyCreditSales ?? 0) + 
            (DailySalesDiscount ?? 0) + (DailyCreditSalesTax ?? 0);
            
        public decimal DailyPurchaseTotal => 
            (DailyCashPurchase ?? 0) + (DailyCashPurchaseTax ?? 0) + (DailyCreditPurchase ?? 0) + 
            (DailyPurchaseDiscount ?? 0) + (DailyCreditPurchaseTax ?? 0);
    }
