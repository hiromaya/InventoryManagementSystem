...
try
{
        ...
        static void DisplayHelp(string[] args)
        {
            ...
            Console.WriteLine("📊 在庫管理システム - コマンドライン インターフェース");
            ...
            Console.WriteLine("  import-payment <file> [YYYY-MM-DD]          - 支払伝票CSVを取込");
            Console.WriteLine("  import-receipt <file> [YYYY-MM-DD]          - 入金伝票CSVを取込");
            Console.WriteLine("  import-folder [YYYY-MM-DD] [フォルダパス]    - フォルダ内の全CSVファイルを一括取込");
            Console.WriteLine("  import-masters [フォルダパス]                - マスタファイルを一括取込");
            Console.WriteLine();
            
            Console.WriteLine("🛠️ システム管理:");
            Console.WriteLine("  init-database                               - データベースを初期化");
            Console.WriteLine("  test-connection                             - データベース接続をテスト");
            Console.WriteLine("  check-masters                               - マスタデータ整合性チェック");
            Console.WriteLine("  test-fastreport                             - FastReportテスト（DB不要）");
            Console.WriteLine();

            Console.WriteLine("🧪 開発・テスト用:");
            Console.WriteLine("  dev-daily-close <YYYY-MM-DD> [--skip-validation] [--dry-run] - 開発用日次終了処理");
            ...
            Console.WriteLine("  dev-daily-report <YYYY-MM-DD> [--skip-unmatch-check] - 開発用商品日報（制限無視）");
            Console.WriteLine("  dev-product-account <YYYY-MM-DD> [--skip-unmatch-check] - 開発用商品勘定（制限無視）");
            Console.WriteLine("  dev-inventory-list <YYYY-MM-DD> [--skip-unmatch-check] - 開発用在庫表（制限無視）");
            Console.WriteLine("  dev-check-daily-close <YYYY-MM-DD>         - 開発用日次終了確認（時間制限無視）");
            Console.WriteLine();
        ...
        static void DisplayCommandHelp(string commandName)
        {
            ...
            {
                case "import-folder":
                    Console.WriteLine("📥 import-folder - フォルダ内CSV一括取込");
                    Console.WriteLine();
                    Console.WriteLine("説明:");
                    Console.WriteLine("  指定フォルダ内の全CSVファイルを検出し、ファイル名に基づいて");
                    Console.WriteLine("  適切な取込処理を自動実行します。");
                    Console.WriteLine();
                    Console.WriteLine("使用方法:");
                    Console.WriteLine("  dotnet run import-folder [YYYY-MM-DD] [フォルダパス]");
                    Console.WriteLine();
                    Console.WriteLine("例:");
                    Console.WriteLine("  dotnet run import-folder 2025-06-16");
                    Console.WriteLine("  dotnet run import-folder 2025-06-16 D:\\CSVFiles");
                    Console.WriteLine();
                    Console.WriteLine("対応ファイル名パターン:");
                    Console.WriteLine("  売上伝票.csv, 仕入伝票.csv, 在庫調整伝票.csv");
                    Console.WriteLine("  支払伝票.csv, 入金伝票.csv");
                    Console.WriteLine();
                    Console.WriteLine("出力ファイル:");
                    Console.WriteLine("  D:\\InventoryBackup\\Reports\\ImportResults_YYYYMMDD.txt");
                    break;
                ...
                case "unmatch-list":
                    ...
                    Console.WriteLine();
                    Console.WriteLine("例:");
                    Console.WriteLine("  dotnet run unmatch-list 2025-06-16");
                    Console.WriteLine();
                    Console.WriteLine("チェック項目:");
                    Console.WriteLine("  - 在庫マスタに存在しない商品の伝票");
                    Console.WriteLine("  - 伝票に存在しない商品の在庫");
                    Console.WriteLine("  - 数量・金額の不整合");
                    Console.WriteLine();
                    Console.WriteLine("出力ファイル:");
                ...
                case "daily-report":
                    ...
                    Console.WriteLine();
                    Console.WriteLine("説明:");
                    Console.WriteLine("  指定日の商品別売上・仕入・在庫状況をまとめた日報をPDF形式で出力します。");
                    Console.WriteLine("  アンマッチ0件が前提条件です。");
                    Console.WriteLine();
                    Console.WriteLine("使用方法:");
                    Console.WriteLine("  dotnet run daily-report <YYYY-MM-DD> [--dataset-id ID]");
                    Console.WriteLine();
                    Console.WriteLine("例:");
                    Console.WriteLine("  dotnet run daily-report 2025-06-16");
                    Console.WriteLine();
                    Console.WriteLine("レイアウト:");
                    Console.WriteLine("  - A3横向き（420mm × 297mm）");
                    Console.WriteLine("  - 日計・月計の売上、粗利、粗利率を表示");
                    Console.WriteLine("  - 商品分類別集計");
                    Console.WriteLine();
                    Console.WriteLine("出力ファイル:");
                ...
                case "product-account":
                    ...
                    Console.WriteLine("レイアウト:");
                    Console.WriteLine("  - A3横向き（420mm × 297mm）");
                    Console.WriteLine("  - 商品別の仕入・売上・残高履歴");
                    Console.WriteLine("  - 担当者別グループ化");
                    Console.WriteLine();
                    Console.WriteLine("出力ファイル:");
                    Console.WriteLine("  D:\\InventoryBackup\\Reports\\ProductAccount_YYYYMMDD.pdf");
                    break;
        ...
        private static async Task ExecuteDevProductAccountAsync(IServiceProvider services, string[] args)
        {
            ...
            private static async Task ExecuteUnmatchListAsync(IServiceProvider services, string[] args)
            {
                using (var scope = services.CreateScope())
                {
                    var scopedServices = scope.ServiceProvider;
        ...
        static async Task ExecuteInventoryListAsync(IServiceProvider services, string[] args)
{
        ...
        using (var scope = services.CreateScope())
        {
            ...
            try
            {
                ...
                // 1. 仮テーブル設計確認
                Console.WriteLine("📊 仮テーブル設計でCP在庫マスタを管理します");
                ...
                await cpInventoryRepository.CalculateDailyStockAsync();
                await cpInventoryRepository.SetDailyFlagToProcessedAsync();
                Console.WriteLine("✅ CP在庫マスタ作成完了（仮テーブル）");
